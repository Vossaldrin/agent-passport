    ...
    def pay(self, url: str, amount: float, description: str = "Autonomous payment"):
        ...
        try:
            with x402_requests(self.x402_client) as session:
                response = session.get(url)   # Change to .post() if needed
                print(f"✅ x402 Payment + Request Successful! Status: {response.status_code}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Agent Passport with x402 Payments")
